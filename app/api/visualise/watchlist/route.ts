...
function buildForecastEvents(forecast: ForecastPoint[]): VisualiseDatasetEvent[] {
  return forecast.reduce<VisualiseDatasetEvent[]>((events, point) => {
    ...
    events.push({
      ...
      attribute: {
        weather_risk_percentage: point.riskScore,
      },
    })
...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json()) as { hubs?: HubForecast[] }
    const hubs = Array.isArray(body.hubs) ? body.hubs : []

    const datasets = hubs.reduce<
      Array<{
        datasetName: string
        attributeName: string
        events: VisualiseDatasetEvent[]
      }>
    >((items, hub) => {
      const forecast = Array.isArray(hub.forecast) ? hub.forecast : []
      ...
      items.push({
        datasetName: hub.hubName?.trim() || hub.hubId?.trim() || "Subscribed Hub",
        attributeName: "weather_risk_percentage",
        events,
      })
    ...
    const visualiseResponse = await visualiseApiRequest<VisualiseResponse>("/visualise", {
      method: "POST",
      body: JSON.stringify({
        title: "Watchlist Weather Risk Trend",
        yAxisTitle: "Weather Risk Score (%)",
        returnURL: true,
        datasets,
      }),
    })
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to generate watchlist visualisation.",
      },
      { status: 500 }
    )
  }
